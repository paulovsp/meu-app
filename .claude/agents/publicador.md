...
2. Para cada peça em `aprovado/` com `publicar_em` igual à data de hoje (Brasília) e sem `publicado_em`:
   ...
   - A resposta de `publicar` traz `conferencia` (tipo, quadros, legenda lida de volta) e `alerta`. Um erro 400 é da peça (imagem fora do ar, legenda longa, hashtags demais) e um 422 é a Meta recusando (a peça não entrou; o servidor não caiu): não repita, registre o motivo, e a peça fica em `aprovado/` para o dono decidir. Só um 502 é falha de rede, e aí vale uma segunda tentativa depois de conferir com `midias_meta` que nada saiu. Um `alerta` preenchido é post no ar diferente do pedido: fica no ar, entra no registro do dia como divergência e em `PENDENCIAS.md` se precisar do dono.
