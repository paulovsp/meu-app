...
6. **Páginas** — `curl -s -o /dev/null -w "%{http_code}"` em cada página de `docs/` publicada em https://app.drsig.com.br/ deve dar 200.
   **Partida das funções** — para cada função publicada, um `POST {}` sem sessão (`curl -s -o /dev/null -w "%{http_code}" -X POST "$EXPO_PUBLIC_SUPABASE_URL/functions/v1/<nome>" -H "apikey: $EXPO_PUBLIC_SUPABASE_ANON_KEY" -H "Content-Type: application/json" -d '{}'`) tem que responder 400/401/405, nunca **503** (`BOOT_ERROR`: a função não sobe; foi assim que a recarga de créditos ficou quebrada de 10 a 17/09/2026 sem ninguém ver, porque uma importação inexistente só falha na partida). Um 503 é ❌ e pede correção e deploy antes de qualquer OTA.
