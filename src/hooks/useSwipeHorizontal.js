...
export function useSwipeHorizontal({
  ...
  // Quando existe um número fixo de telas (ex: Início tem só Clínica ⇄
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_evt, gestureState) => {
        const noLimite = (gestureState.dx < 0 && !podeEsquerda) || (gestureState.dx > 0 && !podeDireita);
        translateX.setValue(noLimite ? gestureState.dx * 0.3 : gestureState.dx);
      },
      onPanResponderRelease: (_evt, gestureState) => {
        const indoEsquerda = gestureState.dx <= -limiar && podeEsquerda;
        const indoDireita = gestureState.dx >= limiar && podeDireita;
